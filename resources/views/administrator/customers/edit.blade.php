    ...
    <div class="content d-flex flex-column flex-column-fluid" id="kt_content">
        ...
        <div class="post d-flex flex-column-fluid" id="kt_post">
            <div id="kt_content_container" class="container-xxl">
                <form class="form d-flex flex-column flex-lg-row" id="form" method="post" enctype="multipart/form-data" action="{{ route('admin.customers.update', $customers->id) }}">
                    {{ csrf_field() }}
                    {{ method_field('PUT') }}
                    <div class="d-flex flex-column flex-row-fluid gap-7 gap-lg-10">
                        <div class="card card-flush py-4">
                            ...
                            <div class="card-body pt-0">
                                <div class="fv-row">
                                    <label class="form-label required">Customer Group</label>
                                    <select name="customer_group" class="form-select" data-control="select2" data-placeholder="Select an option">
                                        <option value="">Please Select</option>
                                        @foreach ($customer_group as $row) {
                                            <option value="{{ $row->id }}" @if($row->id == $customers->customer_group_id) selected @endif > {{ $row->name }} </option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="card-body pt-0">
                                <div class="fv-row">
                                    ...
                                    <input type="text" name="name" class="form-control mb-2" placeholder="Name" value="{{ $customers->name }}"/>
                                </div>
                            </div>
                            <div class="card-body pt-0">
                                <div class="fv-row">
                                    <label class="required form-label">Email</label>
                                    <input type="email" name="email" class="form-control mb-2" placeholder="Email" value="{{ $customers->email }}"/>
                                </div>
                            </div>
                            <div class="card-body pt-0">
                                <div class="fv-row">
                                    <label class="required form-label">Phone</label>
                                    <input type="text" name="phone" class="form-control mb-2" placeholder="Phone" value="{{ $customers->phone }}"/>
                                </div>
                            </div>
                            <div class="card-body pt-0">
                                <div class="fv-row">
                                    <label class="required form-label">Address</label>
                                    <textarea class="form-control" name="address" placeholder="Address">{{ $customers->address }}</textarea>
                                </div>
                            </div>
                            <div class="card-body pt-0">
                                <div class="fv-row">
                                    <label class="form-label">Password</label>
                                    <input type="password" name="password" id="password" class="form-control mb-2" placeholder="Password" value=""/>
                                </div>
                            </div>
                            <div class="card-body pt-0">
                                <div class="fv-row">
                                    <label class="form-label">Confirm Password</label>
                                    <input type="password" name="confirm_password" class="form-control mb-2" placeholder="Confirm Password" value=""/>
                                </div>
                            </div>
                            <div class="card-body pt-0">
                                <div class="fv-row">
                            ...
                            <div class="card-body pt-0">
                                <div class="fv-row">
                                    ...
                                    <div class="form-check form-switch form-check-custom form-check-solid">
                                        <input class="form-check-input" type="checkbox" value="1" id="status" name="status" @if($customers->status == 1) checked="checked" @endif />
                        ...
                        <div class="d-flex justify-content-end">
                            <a href="{{ route('admin.customers') }}" class="btn btn-light me-5">Cancel</a>
                            <button type="submit" id="customer_submit" class="btn btn-primary">
...
@push('scripts')
    ...
    <script src="{{ asset_administrator('assets/js/custom/apps/ecommerce/catalog/save-  .js') }}"></script>
