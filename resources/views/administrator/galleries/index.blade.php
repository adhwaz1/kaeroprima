            ...
            <div id="kt_content_container" class="container-xxl">
                ...
                <div class="card card-flush">
                    <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                        ...
                        <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                            ...
                            <div class="card-toolbar">
                                @if (isAllowed("galleries", "add"))
                                <a href="{{ route('admin.galleries.add') }}" class="btn btn-sm btn-light-primary">
                    ...
                    <div class="card-body pt-0">
                        <table class="table align-middle table-row-dashed fs-6 gy-5" id="gallery-table">
                            <thead>
                                <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
                                    <th class="min-w-10px">No</th>
                                    <th class="min-w-150px">Judul Album</th>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            var data_gallery = $('#gallery-table').DataTable({
                processing: true,
                serverSide: true,
                order: [[0, 'desc']],
                ajax: {
            ...
            $('#searchdatatable').keyup(function() {
                data_gallery.search($(this).val()).draw();
            });

            $('#filterstatus').change(function() {
                data_gallery.search($("#searchdatatable").val()).draw();
            })
            ...
            $(document).on('click', '.delete', function(event) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: "POST",
                            url: "{{ route('admin.galleries.delete') }}",
                            data: ({
                                "_token": "{{ csrf_token() }}",
                                "_method": 'DELETE',
                                ix: ix,
                            ...
                            success: function() {
                                Swal.fire('Deleted!', 'Data has been deleted',
                                    'success');
                                data_gallery.ajax.reload(null, false);
                            }
                        });
