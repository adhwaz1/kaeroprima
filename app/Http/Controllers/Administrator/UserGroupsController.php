...
class UserGroupsController extends Controller
{
    ...
    {
        $query = UserGroup::select(DB::raw('user_groups.*'));

        if ($request->status != "") {
            $status = $request->status == "active" ? 1 : 0;
            $data = $query->where("user_groups.status", $status)->get();
        } else {
            $data = $query->get();
        }
        ...
        return DataTables::of($data)
            ...
            ->addColumn('action', function ($row) {
                ...
                if (isAllowed(static::$module, "edit")) : //Check permission
                    $btn .= '<a href="' . route('admin.user_groups.edit', $row->id) . '" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
    ...
    public function save(Request $request)
    {
        ...
        //Write log
        createLog(static::$module, __FUNCTION__, $user_group->id);
        return redirect(route('admin.user_groups'));
    }
    ...
    public function detail($id)
    {
        ...
        $detail = UserGroup::find($id);
        if (!$detail) {
            return abort(404);
        }

        return view('administrator.user_groups.detail', compact("detail"));
    }
    ...
    public function delete(Request $request)
    {
        ...
        $id = $request->ix;
        $user_group = UserGroup::find($id);
        $data = UserGroup::destroy($request->ix);
        //Write log
        createLog(static::$module, __FUNCTION__, $id);
